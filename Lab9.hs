...
byp One = True
byp (Let' a) = False
byp (Union' rs) = or[byp a | a <- rs]
byp (Cat' rs) = and[byp a | a <- rs]
byp (Star' r1) = True
...
deriv w Zero = Zero
deriv w One = One
deriv w (Let' z) = if w == z then One else Zero
deriv w (Union' rs) = Union' (map (deriv w) rs)
deriv w (Cat' rs) = Union' (helper rs) where
  helper [] = []
  helper (r:rs) = Cat'(deriv w r : helper rs) : if byp r then helper rs else []
deriv w (Star' r) = Cat' [deriv w r, Star' r]
...
conv Zero = undefined--(0, 0, 0, 0)
  


-- Test, and show your tests! You may copy code from previous labs to help.

extendedregular = fromRE (Star (Union (Let 'a') (Let 'b')))
extendedregular1 = fromRE (Union (Let 'a') (Let 'b'))
extendedregular2 = fromRE (Cat (Union (Let 'a') (Let 'b')) (Star (Union (Let 'a') (Let 'b'))))

answer = byp extendedregular -- = True
answer1 = byp extendedregular1 -- = False
answer2 = byp extendedregular2 -- = False

deriv 'a' extendedregular = Cat' [Union' [One,Zero],Star' (Union' [Let' 'a',Let' 'b'])]
deriv 'b' extendedregular = Cat' [Union' [Zero,One],Star' (Union' [Let' 'a',Let' 'b'])]

deriv 'a' extendedregular1 = Union' [One,Zero]
deriv 'b' extendedregular1 = Union' [Zero,One]

deriv 'a' extendedregular2 = Union' [Cat' [Union' [One,Zero],Cat' [Cat' [Union' [One,Zero],Star' (Union' [Let' 'a',Let' 'b'])]]]]
deriv 'b' extendedregular2 = Union' [Cat' [Union' [Zero,One],Cat' [Cat' [Union' [Zero,One],Star' (Union' [Let' 'a',Let' 'b'])]]]]
